...
;(function(window) {
	...
	TiltFx.prototype._initEvents = function() {
		...
		// mousemove event..
		document.querySelector(".mouseeffect-area").addEventListener('mousemove', moveeffect);

    var slowdown = 0;

    function moveeffect(ev) {
      slowdown++;
      if(slowdown >= 10){
        slowdown = 0;
        return;
		...
		document.querySelector(".mouseeffect-area").addEventListener('mouseleave', function(ev) {
			setTimeout(function() {
			for(var i = 0, len = self.imgElems.length; i < len; ++i) {
				var el = self.imgElems[i];
